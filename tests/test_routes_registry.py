...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        # 159 base GET + 25 v1 aliases = 184 total (v2 additions remain
        ...
        # effortless metadata adds /api/v2/metadata/auto-scrape and
        # /api/v2/metadata/scrape-settings).
        self.assertEqual(len(GET_TABLE), 184)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_get), 159)
        self.assertEqual(len(base_post), 175)
        self.assertEqual(len(base_get) + len(base_post), 334)

    def test_all_routes_registered(self):
        routes = all_routes()
        # 335 = 334 table paths + the /api/v1/jobs dual-registration.
        self.assertEqual(len(routes), 335)

        get_routes = [r for r in routes if r.method == "GET"]
        post_routes = [r for r in routes if r.method == "POST"]
        self.assertEqual(len(get_routes), 160)
