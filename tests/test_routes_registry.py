...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        # 158 base GET + 25 v1 aliases = 183 total (v2 additions remain
        ...
        # Game DNA adds /api/v2/library/dna/status).
        # 175 base POST + 43 v1 aliases = 218 total (v2 feature CRUD rows are
        ...
        # Game DNA adds /api/v2/library/dna/search and
        # /api/v2/library/dna/index/rebuild).
        self.assertEqual(len(GET_TABLE), 183)
        self.assertEqual(len(POST_TABLE), 218)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_get), 158)
        self.assertEqual(len(base_post), 175)
        self.assertEqual(len(base_get) + len(base_post), 333)

    def test_all_routes_registered(self):
        routes = all_routes()
        # 334 = 333 table paths + the /api/v1/jobs dual-registration.
        self.assertEqual(len(routes), 334)

        get_routes = [r for r in routes if r.method == "GET"]
        post_routes = [r for r in routes if r.method == "POST"]
        self.assertEqual(len(get_routes), 159)
        self.assertEqual(len(post_routes), 175)
