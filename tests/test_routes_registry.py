...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        # 120 base GET + 25 v1 aliases = 145 total (1.9.0 adds mastery)
        # 115 base POST + 42 v1 aliases = 157 total (1.9.0 adds picker)
        self.assertEqual(len(GET_TABLE), 145)
        self.assertEqual(len(POST_TABLE), 157)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_get), 120)
        self.assertEqual(len(base_post), 115)
        self.assertEqual(len(base_get) + len(base_post), 235)

    def test_all_routes_registered(self):
        routes = all_routes()
        self.assertEqual(len(routes), 236)

        get_routes = [r for r in routes if r.method == "GET"]
        post_routes = [r for r in routes if r.method == "POST"]
        self.assertEqual(len(get_routes), 121)
        self.assertEqual(len(post_routes), 115)
