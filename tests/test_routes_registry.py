...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        ...
        # 1.10.0 adds POST /api/v2/import/launchbox/resolve (+1 POST, v1 frozen at 60).
        self.assertEqual(len(GET_TABLE), 148)
        self.assertEqual(len(POST_TABLE), 165)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_post), 123)
        self.assertEqual(len(base_get) + len(base_post), 246)

    def test_all_routes_registered(self):
        routes = all_routes()
        self.assertEqual(len(routes), 247)
        ...
        self.assertEqual(len(post_routes), 123)
