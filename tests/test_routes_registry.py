...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        ...
        # 165 base POST + 42 v1 aliases = 207 total (v2 feature CRUD rows are
        # additive to the existing query/trash/memories/resume surface;
        # 1.12 adds collections save + delete).
        self.assertEqual(len(GET_TABLE), 178)
        self.assertEqual(len(POST_TABLE), 207)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_post), 165)
        self.assertEqual(len(base_get) + len(base_post), 318)

    def test_all_routes_registered(self):
        routes = all_routes()
        # 319 = 318 table paths + the /api/v1/jobs dual-registration.
        self.assertEqual(len(routes), 319)
        ...
        self.assertEqual(len(post_routes), 165)
