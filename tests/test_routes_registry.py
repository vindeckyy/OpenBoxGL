...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        ...
        # 176 base POST + 43 v1 aliases = 219 total (v2 feature CRUD rows are
        ...
        # library health adds scan, fix, and undo).
        self.assertEqual(len(GET_TABLE), 184)
        self.assertEqual(len(POST_TABLE), 219)
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_post), 176)
        self.assertEqual(len(base_get) + len(base_post), 335)

    def test_all_routes_registered(self):
        routes = all_routes()
        # 336 = 335 table paths + the /api/v1/jobs dual-registration.
        self.assertEqual(len(routes), 336)
        ...
        self.assertEqual(len(post_routes), 176)
