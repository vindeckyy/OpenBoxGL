...
class RouteRegistryTests(unittest.TestCase):
    def test_route_table_sizes(self):
        # 169 base GET + 25 v1 aliases = 194 total (v2 additions remain
        # additive; 1.13 adds household activity, party decks, Time Machine
        # compare, save history, setup checklists, the signed defs channel,
        # background update status, plugin commands, artwork hygiene, missing
        # path repair, duplicate merge, session export, and collection export).
        # 188 base POST + 42 v1 aliases = 230 total (v2 feature CRUD rows are
        # additive to the existing query/trash/memories/resume surface;
        # 1.13 adds presence, deck, save-history, weekly-challenge, wishlist,
        # plugin-command, defs-channel, background-download, repair, duplicate
        # merge, test-restore, and collection import actions).
    ...
    def test_base_routes_count(self):
        ...
        self.assertEqual(len(base_post), 188)
        self.assertEqual(len(base_get) + len(base_post), 357)

    def test_all_routes_registered(self):
        routes = all_routes()
        # 358 = 357 table paths + the /api/v1/jobs dual-registration.
        self.assertEqual(len(routes), 358)
        ...
        self.assertEqual(len(post_routes), 188)
