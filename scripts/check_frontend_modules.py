...
# must name a file that exists. Hardcoding the entry set missed osk.js when
# the Big Box on-screen keyboard shipped as its own script tag.
SCRIPT_TAG_RE = re.compile(r"""<script[^>]*\ssrc=["']/static/([A-Za-z0-9_.\-]+\.js)["']""")
...
def check_graph() -> list[str]:
    ...
    index_entries = set(SCRIPT_TAG_RE.findall(index_text))
    for tag_module in sorted(index_entries - modules):
        problems.append(
            f"index.html loads /static/{tag_module} which does not exist "
            f"(a missing entry script tag silently disables its feature)"
        )

    for module in sorted(modules):
        if module in WORKER_MODULES or module in index_entries:
            continue
...
def write_baseline(modules: set[str], retired: dict[str, str] | None = None) -> None:
    payload = {
        ...
        "freely; removing one fails the gate. See ADR 0060.",
