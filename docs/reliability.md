...
| 8 | Two quick launches of the same game | Launch button disables while starting; second click is a no-op until the first resolves | Manual |
...
| 11 | Offline metadata sync | LBDB sync surfaces an offline state in the job panel with retry | Manual |
| 12 | Steam library on read-only mount | Import reports the permission error with the actual path | Manual |
| 13 | Wrong RetroAchievements / EmuMovies credentials | 401 surfaces as "check credentials in settings", not a generic error | Manual |
...
| 19 | Broken symlink as media path | /api/media 404s cleanly, no traceback | Manual |
| 20 | 20,000-game library | Grid virtualizes; sidebar counts compute once; search debounces | Manual (perf bench) |
| 21 | 300+ character game names | Ellipsis everywhere; no layout break | Manual |
| 22 | Selected game deleted while a dialog is open | Dialogs close or rebind; no stale selectedId crash | Manual |
| 23 | Rapid filter switching during render | No half-rendered state; render reads one consistent AppState snapshot | Manual |
