...
| 7 | Game still running when OpenBox closes | SIGINT/SIGTERM triggers graceful stop of sessions; shutdown drains webhooks | Tested (web_app stop() + test_sessions.py + test_reliability_shutdown.py) |
| 8 | Two quick launches of the same game | Second Play within 1s returns 409 SESSION_ALREADY_STARTING naming the running launch to focus; never two conflicting sessions | Tested (test_sessions.py test_double_launch_lease) |
...
| 11 | Offline metadata sync | LBDB sync failure finishes the metadata.db_sync operation as error (offline named) with retry; job panel mirrors the error | Tested (test_reliability_offline_sync.py, network-stubbed URLError) |
...
| 13 | Wrong RetroAchievements / EmuMovies credentials | 401 INVALID_CREDENTIALS surfaces as "check credentials in settings", not a generic error | Tested (test_reliability_credentials.py, stubbed 401s) |
...
| 22 | Selected game deleted while a dialog is open | Dialogs close or rebind; no stale selectedId crash | Tested (test_dialog_delete_race.py: rebindOpenDialogsToLibrary closes stale game/metadata dialogs, clears ids) |
