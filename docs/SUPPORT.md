...
- The loopback server writes per-launch `server.token` and `server.port` into the data directory (`web_app.py:751-752`) and deletes both on exit; prefer the `X-OpenBox-Token` header over a `token` query parameter.
