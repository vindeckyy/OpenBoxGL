...
OpenBox 1.13.0 keeps the release-gated x86_64 Flatpak path and maintains the
...
## Completed at 1.11.0, carried through 1.13.0 (prep and release gate)
...
- [x] `openbox.metainfo.xml` release history current through 1.13.0.
...
- [x] **Screenshot hosting.** `openbox.metainfo.xml` points all four screenshots at immutable raw GitHub URLs now pinned to the `v1.13.0` tag (refreshed from `v1.11.0` during the 1.13.0 release). If the artwork changes again, refresh the tag or move the images into the Flathub metadata repository before submitting. The docs-site mirror is useful for the README but is not the Flathub submission source of truth.
...
- [x] **Flatpak bundle gating through 1.13.0.** `release-flatpak.yml` built and published the x86_64 bundle against runtime 49. Each future tag must pass the release gate (version sync + `scripts/check_tests.py`) and the attestation job added in 1.13.0 before publish.
