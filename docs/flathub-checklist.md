...
- [x] **Screenshot hosting.** `openbox.metainfo.xml` points all four screenshots at immutable raw GitHub URLs pinned to a **released** tag (`v1.11.0`), so they resolve today and never rot. The docs-site mirror is useful for the README but is not the Flathub submission source of truth; keep these raw URLs or move the images into the Flathub metadata repository when submitting.
      **Invariant:** the tag in those URLs must always be a tag that already exists. When the artwork is regenerated (`scripts/capture_readme_screenshots.py`), repoint the four URLs **in the same commit that changes the images**, at a tag that exists at that moment — which means after the release is cut, not before. Do not pre-point them at an unreleased tag: the images would 404 and `appstreamcli validate --no-net` would not catch it. The 1.11.0 screenshots predate the logo rebrand and the 1.12 UI additions, so regenerate them and repoint at a current released tag (`v1.14.0` or later) before submission. The repo carries nine screenshots locally; the four currently referenced are the Flathub submission set.
