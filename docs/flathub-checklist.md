...
OpenBox 1.10.0 ships a release-gated x86_64 Flatpak bundle and keeps the
...
## Complete in 1.10.0 (prep and release gate)
...
- [x] `openbox.metainfo.xml` release history current through 1.10.0.
- [x] `scripts/capture_readme_screenshots.py` already produces 1920×1080 16:9 screenshots (no extension needed).
- [x] The v1.10.0 release workflow built and published `OpenBox-x86_64.flatpak` against GNOME Platform/SDK 49.

## Remaining manual steps (at submission time)

- [x] **Screenshot hosting.** `openbox.metainfo.xml` now points all four screenshots at immutable raw GitHub URLs for the 1.10.0 tag. The docs-site mirror is useful for the README but is not the Flathub submission source of truth; keep these raw URLs or move the images into the Flathub metadata repository when submitting.
...
- [x] **Flatpak bundle gating for 1.10.0.** `release-flatpak.yml` built and published the x86_64 bundle against runtime 49. Repeat this check for each future release tag.
