...
3. **`scripts/release.sh`** builds one host arch per invocation (or the explicit `OPENBOX_ARCH`) and emits arch-suffixed artifacts: `OpenBox-<arch>.AppImage{,.zsync,.sha256,.sig}` and `OpenBox-<version>-<arch>-sbom.json`. The tag-triggered release workflow invokes that path for both matrix entries. `sign_release.py`, `verify_release.py`, and `gen_sbom.py` are arch-agnostic (they operate on artifact paths passed as arguments).
4. **SBOM naming**: the x86_64 SBOM keeps the legacy unprefixed name `OpenBox-<version>-sbom.json` matching all prior releases; the aarch64 SBOM uses `OpenBox-<version>-aarch64-sbom.json` so the two are distinct. The existing `OpenBox-*-sbom.json` globs in tests and workflows match both.
5. **CI** (`release.yml`, renamed from `release-appimage.yml` in ADR 0048) builds and attests both arches in a matrix: x86_64 on `ubuntu-22.04` (the release glibc baseline), aarch64 on `ubuntu-24.04-arm` (the recommended, non-deprecating GitHub arm image). The publish job signs and uploads both. The regular `ci.yml` gate also covers x86_64 on `ubuntu-latest` and native aarch64 on `ubuntu-24.04-arm`; those runner labels intentionally differ from the release baseline.
