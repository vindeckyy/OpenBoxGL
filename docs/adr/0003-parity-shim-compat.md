...
Date: 2026-08-20
Status: Accepted
Updated: 2026-09-04 — shim deletion landed in 1.9.0; the `_ParityFlatFinder` is the sole flat-import bridge. Correction: the actual shim count was 28, not 22 (the plan's "22" was stale).

## Context

The wide-improvements plan Step 5 proposed deleting the 28 root `parity_*.py` shims and replacing flat `import parity_*` with a lazy `MetaPathFinder` alias in `pkg/parity/__init__.py`. The shims did `sys.modules[__name__] = importlib.import_module("pkg.parity.<name>")`.

## Decision

Keep the 28 shims for one release as a compatibility fallback. `import parity_*` before `import pkg.parity` would fail without the shim file on `sys.path`, so deleting shims now would break standalone `python3 -c "import parity_backup"` and any plugin that does flat imports without first importing `pkg.parity`. The shims already alias `sys.modules["parity_*"]` to `pkg.parity.parity_*` on first flat import.
...
- ~~`runtime_modules.txt` retains both shim and canonical entries (96 lines) for this release.~~ Shims deleted in 1.9.0; `runtime_modules.txt` retains only canonical `pkg/parity/parity_*.py` entries (102 lines).
