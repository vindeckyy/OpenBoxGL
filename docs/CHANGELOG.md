...
- Effortless metadata (Flagship 2): new `POST /api/v2/metadata/auto-scrape`
  ...
  resolve.
- ScreenScraper ROM-hash confidence (ADR 0059): independent MD5-only and
...
- Game DNA search (ADR 0052): offline smart search with a Title | Smart toggle
...
### Added
- **Windows support (ADR 0048).** OpenBox runs natively on Windows alongside
...
- `pkg/platform_compat.py` is the single platform seam the port is built on:
  ...
  resolution. The runtime stays dependency-free — stdlib plus `ctypes`.
