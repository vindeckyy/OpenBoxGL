...
EXPORT_EXCLUDED_TOP_LEVEL_KEYS = frozenset({
    "previews",
    "operation_history",
    "operations",
})
...
def detach_state_view(view):
    """Convert MappingProxyType trees into plain dicts/lists for export."""
    if isinstance(view, types.MappingProxyType):
        view = dict(view)
    if isinstance(view, dict):
        return {key: detach_state_view(value) for key, value in view.items()}
    if isinstance(view, list):
        return [detach_state_view(item) for item in view]
    return view
...
def redact_state_for_export(state: dict) -> dict:
    """Return a detached copy of state with credentials redacted from settings."""
    if not isinstance(state, dict):
        return {}

    cleaned = copy.deepcopy(detach_state_view(state))
    for key in EXPORT_EXCLUDED_TOP_LEVEL_KEYS:
        cleaned.pop(key, None)
