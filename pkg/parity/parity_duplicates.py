...
* ``identity`` — the same stable ``game_id`` on two records.
    ...
    *,
    ...
    for index, game in enumerate(games):
        if not isinstance(game, dict):
            continue
        identity = str(game.get("game_id") or "")
        ...
        for kind, bucket in buckets:
            if kind == KIND_IDENTITY and identity:
                bucket.setdefault(identity, []).append((index, game))
            elif kind == KIND_PATH and path_key:
    ...
    for kind, bucket in buckets:
        for key in sorted(bucket):
            entries = [entry for entry in bucket[key] if entry[0] not in claimed]
            if len(entries) < 2:
                continue
            if len(groups) >= limit:
                truncated = True
    ...
    return {
        "groups": groups,
        "count": len(groups),
        "truncated": truncated or any(len(bucket) > MAX_GROUP_GAMES for _kind, bucket in buckets),
    }
