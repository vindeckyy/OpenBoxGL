...
OpenBox tracks LaunchBox feature parity for Linux environments. For contribution and release workflow, see [CONTRIBUTING.md](CONTRIBUTING.md) and [CHANGELOG.md](CHANGELOG.md). The latest release is **v0.7.0**, with backup and restore reliability, cloud sync conflict resolution, atomic plugin updates, and a hardened test runner (see the CHANGELOG).
