...
OpenBox tracks LaunchBox feature parity for Linux environments. For contribution and release workflow, see [CONTRIBUTING.md](CONTRIBUTING.md) and [CHANGELOG.md](CHANGELOG.md). The latest release is **v0.8.0**, with handheld performance profiles that apply per-launch-profile TDP limits via ryzenadj on Steam Deck and handhelds (see the CHANGELOG).
