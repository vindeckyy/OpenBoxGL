...
OpenBox tracks LaunchBox feature parity for Linux environments. For contribution and release workflow, see [CONTRIBUTING.md](CONTRIBUTING.md) and [CHANGELOG.md](CHANGELOG.md). The latest release is **v1.0.1**, the hardening release that fixes frontend regressions, closes security gaps, and makes launch failures visible on SteamOS (see the CHANGELOG).
