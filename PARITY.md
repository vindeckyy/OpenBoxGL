...
OpenBox tracks LaunchBox feature parity for Linux environments. For contribution and release workflow, see [CONTRIBUTING.md](CONTRIBUTING.md) and [CHANGELOG.md](CHANGELOG.md). The latest release is **v1.0.0**, the native-first release that renders the one UI in a WebKitGTK window instead of a browser tab (see the CHANGELOG).
