...
import { refresh } from './library.js';
    ...
    async function openMediaManager() {
      if (!$('mediaManagerDialog').open) {
        const { openDialog } = await import('./dialogs.js');
        openDialog($('mediaManagerDialog'));
      }
      try {
    ...
    async function captureScreenshot(id) { try { const result = await api('/api/screenshot',{method:'POST',body:JSON.stringify({id})}); await refresh(); notify(`Screenshot saved to ${result.path}`); } catch(error) { notifyError(error); } }
    async function downloadBezel(platform) { if (!platform) return notify('Select a game with a platform first'); try { const result = await api('/api/bezels/download',{method:'POST',body:JSON.stringify({platform})}); notify(`Bezels downloaded to ${result.path}`); } catch(error) { notifyError(error); } }
