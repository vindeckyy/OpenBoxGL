...
import { AppState, api, notify, token, selectedIds, playlistFor, applySidebarVisibility } from './state.js';
    ...
    async function openFeature(mode) {
      ...
      try {
        if (mode === 'queue') {
          ...
          $('queueAdd').onclick = async () => { if (AppState.selectedId === null) return notify('Select a game first.'); const game = AppState.games[AppState.selectedId]; try { await api('/api/queue',{method:'POST',body:JSON.stringify({action:'enqueue',game_ids:[game.game_id]})}); notify('Game added to queue'); openFeature('queue'); } catch (error) { notify(error.message); } };
          ...
          $('saveFeatureTags').onclick = async () => { if (AppState.selectedId === null) return notify('Select a game first.'); const tags = $('featureTagInput').value.split(',').map(value => value.trim()).filter(Boolean); try { await api('/api/tags',{method:'POST',body:JSON.stringify({ids:[AppState.games[AppState.selectedId].game_id],tags})}); await refresh(); openFeature('tags'); notify('Tags saved'); } catch (error) { notify(error.message); } };
