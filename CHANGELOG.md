...
- Plugin execution is now sandboxed with bubblewrap when `bwrap` is available, running plugins with no home directory or network access. When the sandbox cannot start, enabled plugins are skipped with a warning rather than running unsandboxed; set `OPENBOX_ALLOW_UNSANDBOXED_PLUGINS=1` to allow trusted plugins to run unsandboxed.
