...
class LibrarySyncUnavailable(CloudSyncError):
    """Full-library synchronization is disabled until a safe transport exists."""

    code = "LIBRARY_SYNC_UNAVAILABLE"

    def __init__(self, message="Full library synchronization is unavailable pending a safe replacement."):
        super().__init__(message)
...
# Legacy full-library protocol is disabled; preserve its filename for recovery.
# See ADR 0038. Statistics sync above remains independently supported.
...
def publish_library(state, folder, device_id="local", now=None):
    """Reject the unsafe legacy whole-library publication protocol."""
    raise LibrarySyncUnavailable()


def pull_library(state, folder, device_id="local", now=None):
    """Reject the unsafe legacy whole-library pull protocol."""
    raise LibrarySyncUnavailable()
