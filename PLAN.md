...
`index.html:774` says app mode "Falls back to a browser tab when no Chromium-family browser is installed." The code also falls back to `xdg-open` when Chromium exists but fails to launch. Rewrite the description to match: "Applies the next time OpenBox launches. Falls back to your default browser when no compatible browser can open a chrome-less window."

### 5.2 Add `--window`/`--no-window` passthrough in `openbox.sh` and `openbox-native.sh`

Both scripts already pass `"$@"`. The Tk launcher (`openbox.py`) and `openbox.sh` should pass launch mode through: add the flags to the script arguments where the AppImage entrypoints invoke `web_app.py`, and document in the metainfo release entry. No new parser code is needed in `web_app.py` because `--app-window`/`--no-app-window` already exist there; this step only makes the flag reachable from the shipped launcher entrypoints if it is not already (check `io.openbox.GameLauncher.yml` and `build_appimage.sh` for which entrypoint the desktop file uses and pass the flags there).

### 5.3 Docs

- README: add one line under the launch section describing the app window default and the `ui_window` setting / flags.
- Docs site `content/docs/reference/project/releasing.md` is release-only; instead update `content/docs/` pages that document settings and the UI (the settings reference page) with the `ui_window` option, and update the roadmap "In the current release" paragraph to mention the chrome-less app window and the expanded media types, per the existing roadmap-update convention.
